...
#define OPENING "%s: Waiting for Connection (F1 to cancel) ...\n", host
...
enum { PLACE, REMOVE };
static enum { OFF, ON, MOUSE_UP, MOUSE_DOWN } input_active = MOUSE_DOWN;
static mouse_click=0, mouse_click_x, mouse_click_y;
/* events end */
...
static int mx, my, mf;
	...
	if (input_active == MOUSE_DOWN)
		return;
	if (input_active == ON)
	{
		input_active = MOUSE_UP;
...
{	int color, bgcolor, x, y, w, h, ww, i, j, n, len, wr_mode;
	unsigned bit;
	unsigned char *p, *q;
	Pixmap *pixmap;
...
{	struct pm pm;
	...
	while(len--)
	{
		...
		{
			pm.x = x;
			pm.y = y + font->h - font->pixels[*buf].h;
			pm.pixmap = &(font->pixels[*buf]);
			Xput_pixmap(&pm);
/*
...
{	char ch;
	...
	assert(handleReply == 0);
	handleReply = 1;
	Reply.type = Y_FLUSH_INPUT;
	Reply.data.escape = escape;
}
...
{	int input;
	...
	if (input && input_active == MOUSE_DOWN)
		input_active = ON;
	if (!input && input_active == MOUSE_UP)
		input_active = OFF;
	check_mouse();
	...
	{	unsigned char buf[3];
	case X_NIX:
		/* schaltet einfach input auf aktiv */
		break;
	case X_OPEN_DISPLAY:
		Xopen_display();
		break;
	...
	case X_PUT_TEXT:
		...
		break;
	case X_CLIP:
		g_clip(X->data.i[0], X->data.i[1], X->data.i[2], X->data.i[3]);
		break;
	case X_SETPAL:
		buf[0] = X->data.i[0];
		buf[1] = X->data.i[1];
...
{	char ch;
	...
	if (-1 != (ch = _read_kbd(0,0,0)))
	{
		...
		Reply.type = Y_KEYBOARD_EVENT;
		Reply.data.event.ch = ch;
		handleReply = 1;
		return;
	}
	if (mouse_click)
	{
		Reply.type = Y_MOUSE_EVENT;
		Reply.data.event.x = mouse_click_x;
		Reply.data.event.y = mouse_click_y;
		mouse_click = 0;
		handleReply = 1;
		return;
	}
...
typedef struct a_screen {
    short help_on;	/* is help window visible? has 4 states! */
    short escape;	/* for exit_robot_check */
    short xsize, ysize, colors;
    enum repl event_type;
    struct event event;	/* a remote event */
...
static void send(Side *side, struct Xmit *X, enum repl wait)
{
	...
	else
	{
		...
		{	FILE *f;
			...
			if (fread(&Reply, sizeof(Reply), 1, f))
			{
				if (sd()->event_type == Y_NONE &&
				    (Reply.type == Y_KEYBOARD_EVENT ||
				     Reply.type == Y_MOUSE_EVENT))
				{
					sd()->event = Reply.data.event;
					sd()->event_type = Reply.type;
				}
			}
			fclose(f);
			sd()->file = fopen(sd()->in, "wb"); /* clear it */
			assert(sd()->file);
		}
...
static void get_event(Side *side)
{	int rawx, rawy;
	Window window;

	clipwin(side, 0);
	sd()->escape = FALSE;
	if (sd()->in == NULL && sd()->event_type == Y_NONE) /* local host */
	{	int n;
	...
	switch(sd()->event_type)
	{
	case Y_NONE:
		break;
	case Y_KEYBOARD_EVENT:
		sd()->event_type = Y_NONE;
		side->reqtype = KEYBOARD;
		side->reqch = sd()->event.ch;
		break;
	case Y_MOUSE_EVENT:
		sd()->event_type = Y_NONE;
		window = get_window(side, sd()->event.x, sd()->event.y, &rawx, &rawy);
		if (window == side->map) {
			side->reqtype = MAPPOS;
			side->reqch = 1;
...
{	char ch;
	...
	if (active_display(side))
	{
		X.type = X_FLUSH_INPUT;
		send(side, &X, Y_FLUSH_INPUT);
		sd()->escape = Reply.data.escape;
	}
	side->reqchange = TRUE;
}	
