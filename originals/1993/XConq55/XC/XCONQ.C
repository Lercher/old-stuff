...
char *argv[];
{
    ...
    programname = argv[0];
    if ((void *) (xconqlib = (char *) getenv((char *) "XCONQLIB")) == NULL)
#ifdef MSDOS
        {
        char *cptr;

        cptr = (char *) strrchr((char *) programname, (int) '\\');
        if (cptr != NULL)
          {
          char c;
          ...
          c = *cptr;
          *cptr = '\0';
          xconqlib = (char *) malloc(strlen(programname) + 1);
          if (xconqlib == NULL)
            {
            xconqlib = XCONQLIB;
            }
...
usage_error()
{
    fprintf(stderr, "Usage: %s [display] [-A display] [-B] [-C] [-e n] [-m name]\n",
	    programname);
    fprintf(stderr, "  [-M w h] [-p name] [-r] [-s name] [-t n] [-v]\n");
    exit(1);
}
