...
Side *side;
{
    ...
    case '+':
    case 'n':
	if (helpstate < 5+period.numutypes) helpstate++;
	break;
    case '\177':
    case '\b':
    case '-':
    case 'p':
	if (helpstate > 0) helpstate--;
	break;
    default:
	conceal_help(side);
	...
	describe_utype(side, helpstate - 6);
	break;
    }
    ask_help_char(side);
#if 0
...
Side *side;
{
	...
	if (u != NOTHING) {
	  ...
	  else find_unit_by_number(side, u, helpstate);
	}
	clear_prompt(side);
    } else {
	request_input(side, (Unit *) NULL, x_find_unit);
    }
}
...
int n;
{
  if (side->mode != SURVEY) {
    ...
    ask_unit_type(side, "Find which type of unit?", (short *) NULL);
    request_input(side, (Unit *) NULL, x_find_unit);
    helpstate = n;
  }
}
...
Side *side;
{
	...
	if (u != NOTHING) {
		...
		ask_help_char(side);
		helpstate = u;	/* -ml  reversed lines < & v */
		request_input(side, (Unit *) NULL, x_help);
	    }
	}
	clear_prompt(side);
    } else {
	request_input(side, (Unit *) NULL, x_unit_info);
    }
}
...
int n;
{
    ask_unit_type(side, "Details on which unit type?", (short *) NULL);
    request_input(side, (Unit *) NULL, x_unit_info);
}
